...
namespace PerPlayerLoot
{
    ...
    public class PPLPlugin : TerrariaPlugin
    {
        ...
        public override string Description => "为每个玩家复制战利品箱库存.";
        #endregion
        ...
        private void ToggleCommand(CommandArgs args)
        {
            enablePpl = !enablePpl;
            if (enablePpl) {
                args.Player.SendSuccessMessage("现在启用了每个玩家的战利品!");
            } else {
                args.Player.SendSuccessMessage("每个玩家的战利品现在被禁用！您现在可以修改宝箱，它们将被视为战利品宝箱.");
            }
        }
        ...
        private void OnChestOpen(object sender, GetDataHandlers.ChestOpenEventArgs e)
        {
            ...
            e.Player.SendInfoMessage("这个箱子里的战利品是按玩家保存的!");
